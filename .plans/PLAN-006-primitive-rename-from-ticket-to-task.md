---
id: PLAN-006
title: primitive rename from ticket to task
status: draft
project: ticketbook
...
> **Status:** draft. Scope the primitive rename from ticket -> task across code, API, MCP, and UI, while explicitly avoiding historical repo-content migration and deferring docs as a primitive.
...
- No migration of existing repo-local `.tickets/`, `.plans/`, or historical markdown wording
- No mandatory rewrite of body headings like `## Tasks` to `## Subtasks`
- No long-term compatibility layer preserving both ticket and task public APIs indefinitely
...
- Optimize for a clean post-migration product surface rather than carrying dual ticket/task terminology long term
...
- Plan readers may tolerate legacy `tickets:` frontmatter during the transition, but newly written plans should emit `tasks:`
...
- [ ] Rename core types and helpers from ticket-oriented names to task-oriented names
- [ ] Rename storage/discovery helpers from `.tickets` assumptions to `.tasks` for new/default behavior
...
- [ ] Rename HTTP API routes from `/api/tickets` to `/api/tasks`
...
### 3. UI rename
- [ ] Rename UI routes, state, and components from ticket-oriented names to task-oriented names
...
### 4. Plans integration
- [ ] Rename plan linkage fields and helpers from `tickets` to `tasks`
...
- [ ] Avoid mass-rewriting historical plans/tickets in this repo unless a specific fixture/test requires it
...
1. Should the on-disk default directory change from `.tickets/` to `.tasks/`, or do we keep the old directory name temporarily while renaming the higher-level primitive?
...
3. Should plan frontmatter keep a legacy `tickets:` reader temporarily for old files, even if newly written plans emit `tasks:`?
...
- The system is clean enough to test externally without exposing `ticket` as the primary user-facing primitive
